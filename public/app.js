...
import { createNoticeState, pendingNotices } from './notice-state.mjs';
...
const state = { view: 'overview', data: null, loading: false, ptScope: '' };
...
function renderDailySummary(data) {
  ...
  const title = !m.total ? '等待今日签到数据' : !m.fresh ? '当前数据已过期' : m.allResolved ? '今日签到项已全部确认' : `还有 ${m.pending} 个签到项待处理`;
  append(copy, el('p', 'eyebrow', `DAILY CHECK-IN / ${data?.businessDate ?? '—'}`), el('h2', null, title),
    el('p', 'daily-note', `${m.success} 项成功，${m.unavailable} 项未开放。系统运行正常不代表所有站点签到成功。`));
  const progress = el('div', 'daily-progress');
  append(progress, el('strong', null, m.rate === null ? '—' : `${m.rate}%`), el('span', null, '开放签到项完成率'));
  ...
  $('#sync-age').textContent = `快照生成 ${formatTime(data?.generatedAt)} · ${m.ageMinutes === null ? '尚无数据' : `${m.ageMinutes} 分钟前`} · ${m.fresh ? '数据有效' : '请检查同步'}`;
}

function renderReadiness(gate) {
  const node = $('#readiness-content'); node.replaceChildren();
  const required = Number(gate?.requiredConsecutiveDays) || 7;
  const eligible = Math.min(required, Number(gate?.eligibleRecentDays) || 0);
  const count = el('div', 'readiness-count'); append(count, el('strong', null, eligible), el('span', null, `/ ${required} 天`)); node.append(count);
  const steps = el('div', 'day-steps'); steps.setAttribute('aria-label', `已积累 ${eligible} 个合格观察日，共需 ${required} 天`);
  for (let i = 0; i < required; i++) steps.append(el('i', i < eligible ? 'filled' : ''));
  node.append(steps);
  node.append(el('p', 'readiness-label', gate?.accepted ? '观察验收通过，仍需确认切换' : '观察尚未通过，真实执行保持关闭'));
  const labels = { insufficient_consecutive_days: '近期连续观察天数不足', history_not_current: '最新观察记录已过期', invalid_records: '观察窗口内存在无效记录', health_not_fresh: '观察窗口内存在过期健康数据', health_not_healthy: '观察窗口内存在运行异常', owner_conflict: '执行归属存在冲突', plan_changed_in_window: '近期计划变更，需重新积累稳定记录', no_records: '还没有影子观察记录' };
  const reasons = el('ul', 'readiness-reasons');
  for (const reason of gate?.reasons ?? []) reasons.append(el('li', null, labels[reason] ?? '存在待校验项目'));
  node.append(reasons);
  node.append(el('p', 'boundary-note', 'v1 独占签到 · v2 只读观察，不会重复签到。'));
}

function renderAttention(tasks) {
  const pending = pendingNotices(tasks);
  ...
  toolbar.append(markAll); node.append(toolbar);
  for (const task of pending) {
    const row = el('article', 'attention-item');
    row.classList.toggle('is-read', notices.isRead(task));
    ...
    view.addEventListener('click', () => {
      notices.markRead([task]); renderAttention(tasks);
...
function renderPtStatus(data) {
  ...
  if (view && !view.querySelector('.scope-banner')) {
    ...
    append(banner, el('strong', null, '监控与签到隔离'), el('span', null, '书签清单 · Harvest 回执 · V1 回执'), el('span', 'scope-divider', '|'), el('span', null, '仅监测站点不会加入签到或重试队列'));
    view.querySelector('.toolbar')?.before(banner);
  }
  const counts = pt.counts ?? {};
  const cards = [
    ['PT 站点', counts.sites ?? 0, `${counts.inLegacyPlan ?? 0} 个在签到计划内`, ''],
    ['仅监测', counts.externalOnly ?? 0, '不参与签到或重试', 'monitor'],
    ['状态新鲜', counts.fresh ?? 0, `共 ${counts.sites ?? 0} 个站点`, 'fresh'],
    ['补签候选', counts.supplementCandidates ?? 0, '仅人工复核，v2 未启用执行', 'review']
  ];
  ...
  for (const site of sites) {
    ...
    const scopeCell = el('td'); append(scopeCell, el('span', 'status-chip', site.inLegacyPlan ? '签到计划内' : '仅监测'), el('span', 'subtext', site.inLegacyPlan ? 'V1 执行' : '执行隔离 · 不补签'));
...
function renderSites(sites) {
  ...
  for (const site of sites) {
    ...
    append(card, top, el('span', 'subtext', host), stats, bar, tasksLink, controls, el('span', 'subtext control-note', 'Beta 中只影响 V2 管理视图，不会暂停旧签到任务。')); grid.append(card);
  }
}
...
function renderSettings(data) {
  ...
  if(observations){
    ...
    append(section,el('h2',null,'V2 只读适配验证'),el('p','muted',`验证时间 ${formatTime(observations.finishedAt)} · ${observations.counts.confirmed}/${observations.counts.total} 项取得明确观察证据。非签到执行验收，不改变 V1 结果。`));
...
function renderAll() {
  ...
  const quality=data.snapshot?.evidenceQuality;
  integrity.textContent=`计划对账：缺少回执 ${reconciliation?.missingCount??0} · 身份冲突 ${reconciliation?.conflictCount??0} · 计划外回执 ${reconciliation?.unexpectedCount??0}。${quality?.unverifiedSuccess??0} 项 V1 成功结论待补结构化证据；面板不会据此补签。`;
