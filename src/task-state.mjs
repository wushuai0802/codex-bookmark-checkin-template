...
export function createTaskInstance({origin,accountKey,businessDate,planHash,adapterId}={}){
 ...
 const taskId=`task_${digest({origin,accountKey,businessDate,adapterId})}`;
 return {schemaVersion:1,taskId,origin,accountKey,businessDate,planHash,adapterId,phase:'planned',mutationCount:0,events:[]};
}
export function prepareSubmission(task,{at=new Date().toISOString()}={}){
 ...
 return {...prepared,intent:{id:`intent_${digest({taskId:task.taskId,planHash:task.planHash})}`,taskId:task.taskId,idempotencyKey:`idem_${digest({taskId:task.taskId,businessDate:task.businessDate})}`,createdAt:new Date(at).toISOString(),mutationAllowed:true}};
}
