...
export function executionBindingForOrigin({origin,config={},strict=false}={}){
  ...
  if(site==='https://agentrouter.org'){
    ...
    return {adapterId:'oauth-reward.execute.v1',adapterRule:{selfPath:raw.selfPath??'/api/user/self',logPath:raw.logPath??'/api/log/self',logType:raw.logType??4,successText:raw.successText??'每日签到成功，增加额度',rewardAmount:raw.rewardAmount??25,forceLogout:raw.forceLogout!==false,logoutPath:raw.logoutPath??'/api/user/logout',logoutPagePath:raw.logoutPagePath??'/console',loginPath:config.oauthLoginUrls?.[site]??'/login',provider:raw.provider??config.automaticOAuthProviders?.[site]??null,userStorageKeys:raw.userStorageKeys,maxLogPages:raw.maxLogPages,verificationWaitMs:raw.verificationWaitMs}};
  }
