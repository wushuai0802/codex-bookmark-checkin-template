...
export async function runObservedTask({adapterDefinition,origin,accountKey,businessDate,planHash,expectedIdentity,context={},allowMutation=false,persistIntent=null,now=new Date().toISOString()}={}){
 ...
 if(expectedIdentity!=null&&String(identity.userId)!==String(expectedIdentity))return {task:transitionTask(task,'blocked',{reason:'identity_mismatch'}),mutationCount:0,stage:'identity'};
 ...
 if(status?.state==='already_done'||status?.state==='not_available'){
  ...
  current=transitionTask(current,status.state,{at:now,evidence:status.evidence,reason:status.reason??null});
   return {task:current,mutationCount:0,stage:status.state};
 }
 ...
 if(!allowMutation)return {task:current,mutationCount:0,stage:'not_signed',dryRun:true};
 ...
 if(submittedState!=='accepted'){
   if(submitted?.actionMayHaveHappened===true||submittedState==='unknown')return {task:transitionTask(submitting,'submission_unknown',{at:now,reason:submitted?.reason??'submission_unknown',evidence:submitted?.evidence??null}),mutationCount:1,stage:'submission_unknown'};
   return {task:transitionTask(current,'blocked',{at:now,reason:submitted?.reason??'submission_rejected'}),mutationCount:0,stage:'submit_rejected'};
 }
 let after;
 ...
 if(after?.state==='confirmed')return {task:successfulVerification(verifying,after.evidence,{at:now}),mutationCount:1,stage:'succeeded'};
 return {task:transitionTask(verifying,'submission_unknown',{at:now,reason:after?.reason??'verification_failed',evidence:after?.evidence??null}),mutationCount:1,stage:'submission_unknown'};
}
