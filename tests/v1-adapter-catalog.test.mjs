...
test('catalog maps related OAuth service and configured attendance URLs to the right families',()=>{
 const c=buildV1AdapterCatalog({
   config:{
     oauthApiCheckinRules:{'https://up.x666.me':{statusPath:'/api/checkin/status'}},
     oauthRecoveryTargetOrigins:{'https://x666.me':'https://up.x666.me'},
     configuredTargets:[{url:'https://p.t-baozi.cc/attendance.php'}]
   },
   plan:{targets:[{origin:'https://x666.me'},{origin:'https://p.t-baozi.cc'}]}
 });
 assert.equal(c.sites.find(site=>site.origin==='https://x666.me').familyId,'oauth-status.v1');
 assert.equal(c.sites.find(site=>site.origin==='https://p.t-baozi.cc').familyId,'native-pt.v1');
});

test('a passive New API probe stays a New API status adapter',()=>{
 const c=buildV1AdapterCatalog({config:{newApiCheckinOrigins:['https://muyuan.do'],nativeChallengePreflight:[{url:'https://muyuan.do/console',passiveOnly:true}]},plan:{targets:[{origin:'https://muyuan.do'}]}});
 assert.equal(c.sites[0].familyId,'new-api-calendar.v1');
});
