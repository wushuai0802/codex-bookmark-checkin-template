...
test('profile verifier promotes only the expected identity without mutation',async()=>{
  ...
  const result=await verifyProfile({profile,root,executablePath:path.join(root,'chrome.exe'),launchPersistentContext:async()=>({pages:()=>[page],close:async()=>{}})});assert.equal(result.state,'ready');assert.equal(result.identity,'7');assert.equal(result.statusVerified,'already_done');
});
