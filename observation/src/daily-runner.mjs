...
import {executionAdapterDefinitions,executionRuleForProfile} from './execution-adapter-registry.mjs';
...
async function runDailyOnce({root=path.resolve('.'),legacyRoot,execute=false,accountKey=null,now=new Date(),runAccount=runCanary,notifyAccount=null,executionLock=null,captchaSolver=null}={}) {
  ...
  for(const file of files){
    ...
    try {
      const adapterRule=executionRuleForProfile({profile,config,adapterRule:migration.adapterRule??{}});
      const task=buildCanaryTask({profile,businessDate:day,planHash,adapterId:migration.adapterId,adapterRule,ownershipState:migration.state}),result=await runAccount({task,execute,root,legacyRoot,executionLock,captchaSolver});
...
export async function runDaily(options={}) {
  if(options.accountKey!=null&&!/^[A-Za-z0-9._-]{1,80}$/.test(options.accountKey))throw Error('invalid account key');
  const root=options.root??path.resolve('.');
  if(loadRuntimeConfig(root).executionEngine==='v1')return runLegacyEngine({root,mode:options.execute?'execute':'dry-run',accountKeys:options.accountKey?[options.accountKey]:[]});
