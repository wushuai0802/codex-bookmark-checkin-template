...
const SESSION_NAME = 'fabric_session';
...
function chooseCanaryResult(current, candidate){
  ...
  const currentScore=current.mode==='canary_execute'&&terminal.has(current.stage)&&current.evidence?.authoritative===true?4:current.mode==='canary_execute'?3:terminal.has(current.stage)?2:1;
  const candidateScore=candidate.mode==='canary_execute'&&terminal.has(candidate.stage)&&candidate.evidence?.authoritative===true?4:candidate.mode==='canary_execute'?3:terminal.has(candidate.stage)?2:1;
...
function mergeCanaryTask(task, receipt, canary){
  const base=publicTask(task,receipt);if(!canary||canary.mode!=='canary_execute'||canary.origin!==task.origin||canary.businessDate!==task.businessDate)return base;
  const terminal=(canary.stage==='succeeded'||canary.stage==='already_done')&&canary.evidence?.authoritative===true;
  const observedStatus=terminal?(canary.stage==='already_done'?'already_signed':'signed'):canary.stage==='not_available'?'not_available':canary.stage==='not_signed'?'not_started':'needs_attention';
...
function buildView(snapshot, ledger, canaryResults = [], runtime = null) {
  ...
  const tasks = (snapshot?.tasks ?? []).map((task) => {
    ...
    if(runtime?.owners.some(item=>item.origin===task.origin&&item.accountRef===task.accountRef)) {
      merged.executionOwner='v2-worker';
      const canary=canaryByTask.get(task.taskId);
      if(!canary||canary.mode!=='canary_execute'||canary.origin!==task.origin||canary.businessDate!==task.businessDate) {
...
function sessionCookie(token, { remember = false, secure = false, clear = false } = {}) {
  const attributes = [
    `${SESSION_NAME}=${clear ? '' : encodeURIComponent(token)}`,
...
export function createDashboardServer({
  ...
  function authorized(request, response) {
    ...
    const cookieToken = requestCookie(request, SESSION_NAME);
  ...
  function loadView() {
    ...
    view.tasks = view.tasks.map(task => {
      ...
      return control?.policy === 'pause'
        ? {...task, attention: {pausedUntil: control.expiresAt}}
        : task;
    });
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (requestUrl.pathname.startsWith('/api/') && rateLimited(request)) {
      sendError(response, 429, 'rate_limited', 'too many requests'); return;
    }
    ...
    if (requestUrl.pathname.startsWith('/api/')) {
      ...
      if (requestUrl.pathname === '/api/overview') {
      ...
      else if (requestUrl.pathname === '/api/config') sendJson(response, 200, { mode: 'shadow_read_only', mutationDisabled: false, executionEnabled: false, executionOwner: (view.execution.owners['v2-worker']??0)>0?'account_scoped':'legacy-checkin', execution:view.execution, authConfigured: authRequired, dataDirectoryConfigured: true });
        ...
        sendJson(response, 200, { sites: activeSiteControls(controls.sites), audit: controls.audit.slice(-30) });
        ...
        try {
          ...
          const expiresAt = pauseExpiresAt(policy, body.pauseHours);
          const controls = readControlState(controlFile);
          controls.sites[origin] = { policy, note, expiresAt, updatedAt: new Date().toISOString(), source: 'dashboard' };
          controls.audit.push({ origin, policy, note, expiresAt, recordedAt: new Date().toISOString(), source: 'dashboard' });
          fs.mkdirSync(path.dirname(controlFile), { recursive: true });
          const temporary = `${controlFile}.${process.pid}.${crypto.randomUUID()}.tmp`;
          try {
            fs.writeFileSync(temporary, `${JSON.stringify(controls, null, 2)}\n`, { encoding: 'utf8', mode: 0o600 });
            fs.renameSync(temporary, controlFile);
          } finally { if (fs.existsSync(temporary)) fs.unlinkSync(temporary); }
          sendJson(response, 200, { ok: true, origin, policy, note, expiresAt, executionImpact: 'none_in_beta' });
