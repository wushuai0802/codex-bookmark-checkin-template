...
export function assertV1Idle(v1Root) {
  ...
  let state=null,pulse=null;
  try {
    if(fs.existsSync(stateFile))state=JSON.parse(fs.readFileSync(stateFile,'utf8'));
    if(fs.existsSync(heartbeat))pulse=JSON.parse(fs.readFileSync(heartbeat,'utf8'));
  } catch {throw Error('V1 scheduler state is unreadable');}
  const running=value=>['running','running_checkin'].includes(String(value?.phase));
  if(running(state))throw Error('V1 scheduler is active');
  if(running(pulse)) {
    const updated=Date.parse(pulse.updatedAt),finished=Date.parse(state?.lastFinishedAt),started=Date.parse(state?.lastAttemptStartedAt);
    // V1's one-shot scheduler can leave its last running pulse after exit.
    // Ignore it only with a dead owner and a later completed run, never age alone.
    const completed=state?.phase==='finished'&&Number.isFinite(updated)&&Number.isFinite(started)&&started<=updated&&updated<=finished&&finished<=Date.now();
    if(!completed||processIsAlive(Number(pulse.processId??pulse.pid))!==false)throw Error('V1 scheduler is active');
  }
}

function assertHandoffWritable(v1Root) {
  const file=path.join(path.resolve(v1Root),'data','v2-integration.json');
  if(!fs.existsSync(file))return;
  let integration;try{integration=JSON.parse(fs.readFileSync(file,'utf8'));}catch{throw Error('V2 integration state is unreadable');}
  if(integration.executionEngine==='v1')throw Error('V2 account handoffs are retired in unified V1 engine mode');
}

export function beginV2AccountHandoff({v1Root,accountKey,origin,expiresAt,now=new Date().toISOString()}={}) {
  assertHandoffWritable(v1Root);
