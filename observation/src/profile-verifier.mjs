...
import {createExecutionAdapter,executionBindingForOrigin,executionRuleForProfile} from './execution-adapter-registry.mjs';
...
export async function verifyProfile({profile,root=path.resolve('.'),planHash,adapterRule={},config={},adapterId=null,executablePath,launchPersistentContext,captchaSolver=null,now=new Date().toISOString()}={}) {
  ...
  const adapter=createExecutionAdapter({adapterId:resolvedAdapterId,origin:profile.origin,rule:executionRuleForProfile({profile,config,adapterRule})});
