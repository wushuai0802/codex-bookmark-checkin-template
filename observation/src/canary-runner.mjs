...
function safeEvidence(result) {
  ...
  return evidence ? {source:String(evidence.source??'none').slice(0,64),authoritative:evidence.authoritative===true,summary:String(evidence.summary||result.task.lastEvent?.reason||'').slice(0,240),...(/^\d{1,20}$/.test(String(evidence.accountId??''))?{accountId:String(evidence.accountId)}:{}),...(/^\d{4}-\d{2}-\d{2}$/.test(String(evidence.businessDate??''))?{businessDate:evidence.businessDate}:{})} : null;
}
...
export async function runCanary({task,execute=false,root=path.resolve('.'),legacyRoot=null,executablePath=null,launchPersistentContext=null,writeOutput=true,recordOutcomeFn=recordOutcome,executionLock=null,captchaSolver=null}={}) {
  ...
  if(runtime.executionEngine==='v1')throw Error('standalone V2 adapter retired; use scripts/run-v1-engine.mjs');
  ...
  report.browserLaunches=result.worker.browserLaunches??1;
  report.oauthDiagnostic=result.worker.oauthDiagnostic??null;
  return writeReport(root,task,report,writeOutput);
}
