...
test('site execution requires one unified lease and keeps the result redacted',async t=>{
  ...
  const result=await runPtSite({root,origin:'https://pt.example',catalogFile:'catalog',catalogHash:'a'.repeat(64),
    acquire:()=>{acquired++;return {owner:{nonce:'synthetic'}};},release:()=>{released++;},
    execute:async options=>{assert.equal(options.origin,'https://pt.example');assert.equal(options.lease.owner.nonce,'synthetic');return {origin:options.origin,status:'signed',observedAt:'2026-09-20T01:00:00Z',evidence:{source:'page_text',authoritative:true,summary:'confirmed'},password:'TEST'};}});
