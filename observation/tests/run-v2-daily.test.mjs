...
function standaloneFixture(prefix) {
  const root=fs.mkdtempSync(path.join(os.tmpdir(),prefix));
  fs.mkdirSync(path.join(root,'config'),{recursive:true});
  fs.writeFileSync(path.join(root,'config','runtime.local.json'),'{"executionEngine":"v2"}');
  return root;
}
...
test('daily runner filters to the selected migration and keeps read-only mode',async()=>{
  const root=standaloneFixture('daily-runner-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  ...
  assert.equal(fs.readFileSync(dailyFile,'utf8'),previous);assert.match(report.output,/observation-acct7\.json$/);
  const executed=await runDaily({root,legacyRoot:legacy,accountKey:'acct7',now:new Date('2026-09-09T00:05:00Z'),execute:true,runAccount:async()=>({stage:'already_done',phase:'already_done',mutationCount:0})});
  assert.equal(fs.readFileSync(dailyFile,'utf8'),previous);assert.match(executed.output,/2026-09-09-acct7\.json$/);
});
...
test('daily runner refuses to start Canary when the execution plan is missing or sentinel-only',async()=>{
  for(const fingerprint of [null,'0'.repeat(64)]){
    const root=standaloneFixture('daily-plan-'),legacy=path.join(root,'legacy');
...
test('daily runner rejects an invalid schedule instead of bypassing its window',async()=>{
  const root=standaloneFixture('daily-schedule-'),legacy=path.join(root,'legacy');fs.mkdirSync(path.join(legacy,'config'),{recursive:true});
...
test('daily runner keeps a successful task when notification delivery fails',async()=>{
  const root=standaloneFixture('daily-notify-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');
  ...
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
  fs.writeFileSync(path.join(outputDir,'v2-profile-registry.json'),JSON.stringify({profiles:[{accountKey:'acct7',origin:'https://fixture.example',state:'ready',identity:'7',expectedIdentity:'7',profileDir:path.join(root,'data','v2-profiles','acct7','chrome-user-data')}]}));
...
test('daily runner reconciles a duplicate successful result without losing its completion time',async()=>{
  const root=standaloneFixture('daily-duplicate-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');
  fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
  fs.writeFileSync(path.join(outputDir,'v2-profile-registry.json'),JSON.stringify({profiles:[{accountKey:'acct7',origin:'https://fixture.example',state:'ready',identity:'7',expectedIdentity:'7',profileDir:path.join(root,'data','v2-profiles','acct7','chrome-user-data')}]}));
  const migrationFile=path.join(outputDir,'migration-acct7.json');fs.writeFileSync(migrationFile,JSON.stringify({state:'candidate',accountKey:'acct7',origin:'https://fixture.example',adapterId:'new-api.execute.v1',ownership:{current:'legacy-checkin'}}));
...
test('one corrupt migration file is isolated while other accounts continue',async()=>{
  const root=standaloneFixture('daily-corrupt-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');
  fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
  fs.writeFileSync(path.join(outputDir,'migration-bad.json'),'not-json','utf8');fs.writeFileSync(path.join(outputDir,'v2-profile-registry.json'),JSON.stringify({profiles:[{accountKey:'acct7',origin:'https://fixture.example',state:'ready',identity:'7',expectedIdentity:'7',profileDir:path.join(root,'data','v2-profiles','acct7','chrome-user-data')}]}));
...
test('migration filename and account identity must match before execution',async()=>{
  const root=standaloneFixture('daily-key-mismatch-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');
  fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
...
test('daily runner blocks migrations whose adapter is not implemented',async()=>{
  const root=standaloneFixture('daily-adapter-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
...
test('daily runner dispatches an implemented non-New-API adapter',async()=>{
  const root=standaloneFixture('daily-agent-adapter-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
  fs.writeFileSync(path.join(outputDir,'v2-profile-registry.json'),JSON.stringify({profiles:[{accountKey:'agentrouter-700001',origin:'https://agentrouter.org',state:'ready',identity:'700001',expectedIdentity:'700001',profileDir:path.join(root,'data','v2-profiles','agentrouter-700001','profile')}]}));
  fs.writeFileSync(path.join(outputDir,'migration-agentrouter-700001.json'),JSON.stringify({state:'candidate',accountKey:'agentrouter-700001',origin:'https://agentrouter.org',accountId:'700001',adapterId:'oauth-reward.execute.v1'}));
...
test('daily runner ignores historical migration progress files',async()=>{
  const root=standaloneFixture('daily-progress-file-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
...
test('daily execution lock covers the entire multi-account orchestration',async()=>{
  const root=standaloneFixture('daily-lock-'),legacy=path.join(root,'legacy'),outputDir=path.join(root,'outputs');fs.mkdirSync(path.join(legacy,'config'),{recursive:true});fs.mkdirSync(path.join(legacy,'data'),{recursive:true});fs.mkdirSync(outputDir,{recursive:true});
  fs.writeFileSync(path.join(legacy,'config','config.json'),JSON.stringify({schedule:'08:05'}));fs.writeFileSync(path.join(legacy,'data','last-valid-bookmark-plan.json'),JSON.stringify({planFingerprint:'a'.repeat(64)}));
