...
import {executionBindingForOrigin,executionRuleForProfile} from '../src/execution-adapter-registry.mjs';
import {assertStandaloneV2Enabled} from '../src/runtime-config.mjs';

assertStandaloneV2Enabled(path.resolve('.'));

const accountKey=process.argv[2];
if(!accountKey)throw Error('explicit account key required');
...
const record={schemaVersion:1,mode:'staged_migration',state:'candidate',accountKey,origin:profile.origin,accountId:profile.identity,
  adapterId:binding.adapterId,adapterRule:executionRuleForProfile({profile,config:legacyConfig}),profileDir:profile.profileDir,v1Fallback:{enabled:true,owner:'legacy-checkin'},
