...
When the NAS has independently changed source files, use exact paths to avoid
overwriting them:

```powershell
.\scripts\deploy-nas-code.ps1 -IncludePaths @('src/dashboard-server.mjs','public/app.js') -WhatIf
.\scripts\deploy-nas-code.ps1 -IncludePaths @('src/dashboard-server.mjs','public/app.js')
```

`-IncludePaths` accepts only existing ordinary files under `src/` or `public/`.
It archives and installs only those paths, backs up all NAS code first, rebuilds
the dashboard with the existing compose files, and restores the backup if the
scoped rebuild or health check fails. Inspect the remote backup path printed
...
- Example public hostname: `checkin.example.com` (replace with your own restricted HTTPS host).
