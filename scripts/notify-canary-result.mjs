...
const receipt={taskId:result.taskId,businessDate:result.businessDate,idempotencyKey:idempotencyKey(result),status:completed?'already_signed':'needs_attention',observedAt:result.completedAt,evidence:{source:'v2_canary',summary,redacted:true}};
...
const delivered=await deliverNotification(existing,{send:async()=>{
  await run(config.executable,['checkin-report','--task-id','fabric_v2_canary','--name','V2 Canary 验收','--source','browser-fabric-v2','--status',completed?'already_done':'needs_attention','--event-key',item.dedupeKey,'--summary',summary,'--occurred-at',result.completedAt],{windowsHide:true,timeout:60000,maxBuffer:65536});
}});
