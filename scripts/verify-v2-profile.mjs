...
const legacyConfigFile=path.join(runtime.legacyRoot,'config','config.json');
const legacyConfig=fs.existsSync(legacyConfigFile)?JSON.parse(fs.readFileSync(legacyConfigFile,'utf8')):{};
const binding=executionBindingForOrigin({origin:registry.profiles[index].origin,config:legacyConfig});
const verified=await verifyProfile({profile:registry.profiles[index],root,planHash:assertPlanHash(plan?.planFingerprint,'profile verification planHash'),config:legacyConfig,adapterRule:binding.adapterRule,adapterId:binding.adapterId,executablePath:runtime.chromeExecutable,launchPersistentContext:(profile,options)=>chromium.launchPersistentContext(profile,options)});
