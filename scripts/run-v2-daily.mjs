...
try {
  ...
  const report=await runDaily({root,legacyRoot:runtime.legacyRoot,execute:process.argv.includes('--execute'),accountKey:accountArg?String(accountArg).trim():null,now,notifyAccount:async(file)=>{const {spawn}=await import('node:child_process');await new Promise((resolve,reject)=>{const child=spawn(process.execPath,[path.join(root,'scripts','notify-canary-result.mjs'),file],{windowsHide:true,stdio:'ignore'});child.once('exit',code=>code===0?resolve():reject(Error(`notification command exited ${code}`)));child.once('error',reject);});}});
