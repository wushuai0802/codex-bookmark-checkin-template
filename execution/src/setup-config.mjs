...
function scopeSelectors(value) {
  return {
    mobileFolderPaths: nonEmptyList(value.mobileFolderPaths),
    mobileFolderIds: nonEmptyList(value.mobileFolderIds),
    mobileFolderGuids: nonEmptyList(value.mobileFolderGuids),
  };
}
...
const hasExactChromeScope = [answers.mobileFolderPaths, answers.mobileFolderIds, answers.mobileFolderGuids]
  .some((value) => nonEmptyList(value).length > 0);
for (const [field, value] of [["mobileFolderNames", answers.mobileFolderNames], ["targetFolderNames", answers.targetFolderNames]]) {
  const requiredByName = field === "mobileFolderNames" && hasExactChromeScope ? false : true;
  if (requiredByName && (!Array.isArray(value) || value.length === 0 || value.some((name) => !String(name).trim()))) {
    throw new Error(`${field} 必须由用户提前确认，并至少包含一个非空文件夹名称；或提供精确书签选择器`);
  }
}
...
const answerScopeSelectors = {};
for (const field of ["mobileFolderPaths", "mobileFolderIds", "mobileFolderGuids"]) {
  const values = nonEmptyList(answers[field]);
  if (values.length > 0) answerScopeSelectors[field] = values;
}
for (const [answerField, configField] of [
  ["edgeMobileFolderPaths", "edgeMobileFolderPaths"],
  ["edgeMobileFolderIds", "edgeMobileFolderIds"],
  ["edgeMobileFolderGuids", "edgeMobileFolderGuids"],
]) {
  const values = nonEmptyList(answers[answerField]);
  if (values.length > 0) answerScopeSelectors[configField] = values;
}
...
if (edgeProfileAnswer && edgeProfileAnswer.toLowerCase() !== "none") {
  ...
  const edgeScopeSelectors = {
    mobileFolderPaths: nonEmptyList(answers.edgeMobileFolderPaths),
    mobileFolderIds: nonEmptyList(answers.edgeMobileFolderIds),
    mobileFolderGuids: nonEmptyList(answers.edgeMobileFolderGuids),
  };
  ...
  Object.assign(selectedEdge, edgeScopeSelectors);
}
