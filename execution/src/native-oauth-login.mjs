...
import { resumeSingleGoogleAccount } from "./google-session-choice.mjs";
...
const checkinMode = oauthAccountCheckinMode(process.argv[10]);
...
const rule = checkinMode === "new_api"
  ? { nativeBrowser: true, logPageUrl: `${origin}/profile`, forceLogout: false, expectedAccountId, verificationWaitMs: 1000 }
  : configuredOAuthReloginRule(origin, runtimeConfig);
...
try {
  ...
  if (reuseExistingDailyEvidence || (checkinMode === "new_api" && existingDailyCheckin?.status !== "login_required")) {
    console.log(JSON.stringify({
      origin,
      provider,
      status: reuseExistingDailyEvidence ? "logged_in" : "needs_attention",
    ...
    const agreementCheckbox = activePage.getByRole("checkbox");
    if (await agreementCheckbox.count() === 1 && !await agreementCheckbox.isChecked()) {
      await agreementCheckbox.check({ timeout: 5000 });
    }
    ...
    const googleAccountChoiceState = { attempted: false };
    ...
    if (/accounts\.google\.com$/i.test(location.hostname)) {
      if (await resumeSingleGoogleAccount(page, googleAccountChoiceState, {
        isolatedIdentity: Boolean(accountKey && expectedAccountId && upstreamProvider === "Google"),
      })) {
        await page.waitForTimeout(1000);
        continue;
      }
  ...
  // Let that request settle before navigating to the authoritative log page.
    ...
    do {
      dailyCheckin = checkinMode === "new_api"
        ? await checkNewApiAccount(page, origin, configuredExpectedAccountId)
        : await tryOAuthReloginCheckinStatus(page, origin, runtimeConfig, "signed");
      if (checkinMode === "new_api") break;
