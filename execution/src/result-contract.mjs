...
export function isConfirmedNotAvailable(result, now = new Date()) {
  ...
  if (result.availabilityKind === "task_disabled") {
    // Account-scoped V2 handoffs use disabledByAccountConfig while the origin
    // itself remains enabled for other accounts.  Both forms are deliberate
    // configuration terminal states and must not be downgraded to the noisy
    // `unconfirmed` result during final report normalization.
    return (result.disabledByConfig === true || result.disabledByAccountConfig === true)
