...
export function shouldStartOAuthFallback(finalUrl, origin, callbackEvidence) {
  try {
    return !callbackEvidence && new URL(finalUrl).origin === new URL(origin).origin && isLoginOrSignInRoute(finalUrl);
  } catch { return false; }
}
...
export function rejectedOAuthCallback(evidence) {
  return Boolean(evidence && (evidence.success !== true || evidence.httpStatus < 200 || evidence.httpStatus >= 300));
}

export function mayClaimOAuthLogin({ origin, finalUrl, callbackEvidence, dailyCheckin, pageLooksAuthenticated }) {
  if (rejectedOAuthCallback(callbackEvidence)) return false;
  if (["login_required", "needs_attention", "deferred", "interactive_challenge"].includes(dailyCheckin?.status)) return false;
  const url = new URL(finalUrl);
  if (url.origin !== new URL(origin).origin) return false;
  if (["signed", "already_signed"].includes(dailyCheckin?.status)) return true;
  // A callback page without a password box has not proved authentication.
  if (/\/(?:api\/)?oauth\//i.test(url.pathname)) return false;
  return pageLooksAuthenticated === true;
}
