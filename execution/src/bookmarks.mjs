...
function folderScopeForSource(source) {
  const paths = selectorList(source?.mobileFolderPaths);
  const ids = selectorList(source?.mobileFolderIds);
  const guids = selectorList(source?.mobileFolderGuids);
  return {
    paths,
    ids,
    guids,
    exact: paths.length > 0 || ids.length > 0 || guids.length > 0,
  };
}

function folderPathValue(folderPath) {
  return folderPath.map((value) => String(value ?? "").trim()).filter(Boolean).join(" / ");
}

function exactFolderMatches(folder, folderPath, scope) {
  if (scope.paths.length > 0 && !scope.paths.includes(folderPathValue(folderPath))) return false;
  if (scope.ids.length > 0 && !scope.ids.includes(String(folder?.id ?? ""))) return false;
  if (scope.guids.length > 0 && !scope.guids.includes(String(folder?.guid ?? ""))) return false;
  return true;
}
    ...
    : [
      {
        ...
        path: bookmarksPath,
        optional: false,
        mobileFolderPaths: options.mobileFolderPaths,
        mobileFolderIds: options.mobileFolderIds,
        mobileFolderGuids: options.mobileFolderGuids,
      },
  ...
  for (let index = 0; index < configured.length; index += 1) {
    ...
    sources.push({
      ...
      targetFolderNames: Array.isArray(source?.targetFolderNames)
        ...
        : null,
      mobileFolderPaths: Array.isArray(source?.mobileFolderPaths)
        ? selectorList(source.mobileFolderPaths)
        : null,
      mobileFolderIds: Array.isArray(source?.mobileFolderIds)
        ? selectorList(source.mobileFolderIds)
        : null,
      mobileFolderGuids: Array.isArray(source?.mobileFolderGuids)
        ? selectorList(source.mobileFolderGuids)
        : null,
...
export async function readBookmarkPlan(bookmarksPath, options = {}) {
  ...
  const globalScope = folderScopeForSource(options);
  if ((mobileNames.size === 0 && !globalScope.exact) || targetNames.size === 0) {
  ...
  for (const document of documents) {
    ...
    const scope = folderScopeForSource(document.source);
    let matchedFolderCount = 0;
    for (const root of Object.values(document.raw.roots ?? {})) {
      walkFolders(root, [], (folder, folderPath) => {
        const matches = scope.exact
          ? exactFolderMatches(folder, folderPath, scope)
          : documentMobileNames.has(nodeName(folder));
        if (matches) {
    ...
    if (scope.exact) {
      exactScopeReports.push({ source: document.source, matchedFolderCount });
      if (matchedFolderCount === 0) {
        throw scopeError(
          "BOOKMARK_SCOPE_MISMATCH",
          `书签精确范围未匹配：${sourceLabel(document.source)}；请检查 mobileFolderPaths/mobileFolderIds/mobileFolderGuids`,
        );
      }
    }
  }
  ...
  for (const mobile of mobileFolders) {
    ...
    const folderKey = mobile.guid || mobile.id || mobile.path;
    const sourceId = bookmarkSourceName ? `${bookmarkSourceName}:${folderKey}` : folderKey;
  ...
  for (const entry of exactMap.values()) {
    ...
    if (Array.isArray(options.excludedOrigins) && options.excludedOrigins.includes(key)) continue;
  ...
  return {
    ...
    exactScopeConfigured: exactScopeReports.length > 0,
    exactScopeReports: exactScopeReports.map(({ source, matchedFolderCount }) => ({
      source: sourceLabel(source),
      paths: selectorList(source.mobileFolderPaths),
      ids: selectorList(source.mobileFolderIds),
      guids: selectorList(source.mobileFolderGuids),
      matchedFolderCount,
    })),
    targets,
  };
...
export async function findBookmarkTarget(bookmarksPath, requestedOrigin, options = {}) {
  ...
  for (const combination of combinations) {
    try {
      ...
      if (error?.code === "BOOKMARK_SCOPE_MISMATCH") throw error;
...
export async function readBookmarkPlanWithBackup(bookmarksPath, options = {}) {
  ...
  for (const combination of combinations) {
    try {
      ...
      if (plan.exactScopeConfigured) {
        throw scopeError(
          "BOOKMARK_SCOPE_EMPTY",
          `书签精确范围已匹配，但签到目标只有 ${plan.targetCount} 个，低于最低 ${minimumTargets} 个；拒绝回退到旧书签备份`,
        );
      }
      failures.push(`${combination.map(sourceLabel).join(" + ")} 中只有 ${plan.targetCount} 个签到目标，低于最低 ${minimumTargets} 个`);
    } catch (error) {
      if (error?.code === "BOOKMARK_SCOPE_MISMATCH" || error?.code === "BOOKMARK_SCOPE_EMPTY") throw error;
      failures.push(error.message);
    }
  }
...
export function publicBookmarkReport(plan) {
  return {
    ...
    sources: plan.sources.map((source) => ({
      ...
      folderGuid: source.folderGuid ?? null,
    ...
    exactScopeConfigured: Boolean(plan.exactScopeConfigured),
    exactScopeReports: plan.exactScopeReports ?? [],
