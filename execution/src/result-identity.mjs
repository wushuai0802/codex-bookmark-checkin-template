...
function executionBinding(value = {}) {
  ...
  const fields = {
    ...
    ...(value?.checkinMode && value.checkinMode !== "oauth_relogin" ? { checkinMode: value.checkinMode } : {}),
    ...(value.boundUpstreamAccountKey ? { boundUpstreamAccountKey: value.boundUpstreamAccountKey, boundUpstreamAccountId: value.boundUpstreamAccountId } : {}),
  };
...
export function accountMetadataForOrigin(origin, config = {}) {
  ...
  const executionAccount = configuredExecutionAccount(config, expectedOrigin);
  ...
  const upstreamProvider = String(raw.upstreamProvider ?? executionAccount?.upstreamProvider ?? config.oauthUpstreamProviders?.[expectedOrigin] ?? "").trim();
  const loginUrl = String(raw.loginUrl ?? config.oauthLoginUrls?.[expectedOrigin] ?? "").trim();
  const automationUserDataDir = String(raw.automationUserDataDir ?? executionAccount?.automationUserDataDir ?? "").trim();
  ...
  return {
    ...
    ...(hasExecutionBinding ? {
      executionBinding: executionBinding({ accountId, provider, upstreamProvider, loginUrl, automationUserDataDir, checkinMode: raw.checkinMode, boundUpstreamAccountKey: executionAccount?.accountKey, boundUpstreamAccountId: executionAccount?.accountId }),
    } : {}),
  };
}
