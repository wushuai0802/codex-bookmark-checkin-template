...
const requestedScope = scopeBase64Index >= 0
  ...
  : scopeIndex >= 0
    ...
    : null;
const scopeProvided = (
  (Array.isArray(requestedScope?.mobileFolderNames) && requestedScope.mobileFolderNames.length > 0)
  || [requestedScope?.mobileFolderPaths, requestedScope?.mobileFolderIds, requestedScope?.mobileFolderGuids]
    .some((value) => Array.isArray(value) && value.length > 0)
)
  && Array.isArray(requestedScope?.targetFolderNames)
  && requestedScope.targetFolderNames.length > 0;
const edgeScopeProvided = (
  (Array.isArray(requestedScope?.edgeMobileFolderNames) && requestedScope.edgeMobileFolderNames.length > 0)
  || [requestedScope?.edgeMobileFolderPaths, requestedScope?.edgeMobileFolderIds, requestedScope?.edgeMobileFolderGuids]
    .some((value) => Array.isArray(value) && value.length > 0)
)
      ...
      try {
        const plan = requestedBrowserScope
          ? await readBookmarkPlanWithBackup(bookmarksPath, {
            ...
            mobileFolderPaths: requestedBrowserScope.mobileFolderPaths,
            mobileFolderIds: requestedBrowserScope.mobileFolderIds,
            mobileFolderGuids: requestedBrowserScope.mobileFolderGuids,
...
const profiles = await inspectProfiles(userDataDir, "Chrome", scopeProvided ? {
  ...
  mobileFolderPaths: selectorList(requestedScope.mobileFolderPaths),
  mobileFolderIds: selectorList(requestedScope.mobileFolderIds),
  mobileFolderGuids: selectorList(requestedScope.mobileFolderGuids),
...
const edgeProfiles = await inspectProfiles(edgeUserDataDir, "Edge", edgeScopeProvided ? {
  ...
  mobileFolderPaths: selectorList(requestedScope.edgeMobileFolderPaths),
  mobileFolderIds: selectorList(requestedScope.edgeMobileFolderIds),
  mobileFolderGuids: selectorList(requestedScope.edgeMobileFolderGuids),
