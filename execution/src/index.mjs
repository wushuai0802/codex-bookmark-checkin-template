...
const integrationSelection = (() => {
  try { return JSON.parse(fsSync.readFileSync(path.join(rootDirectory, "data", "v2-integration.json"), "utf8")); }
  catch { return {}; }
})();
if (integrationSelection.executionEngine === "v1") {
  config.executionEngine = "v1";
  if (!process.argv.includes("--dry-run") && !process.argv.includes("--preflight-origins")) {
    const controller = String(integrationSelection.v2ProjectRoot ?? "");
    if (!path.isAbsolute(controller)) throw new Error("执行网关目录无效");
    const { validateEngineLease } = await import(pathToFileURL(path.join(controller, "src", "legacy-engine.mjs")).href);
    validateEngineLease({ root: controller, legacyRoot: rootDirectory });
  }
}
...
try {
  ...
  if (printPreflightOrigins) {
    ...
    const getSharedContext = async (target) => {
      const runtimeConfig = configForOAuthExecutionAccount(configForOAuthSession(config, oauthSessionProfiles, target.origin), rootDirectory, target.origin);
    ...
    const releaseBoundAccountContext = async (account) => {
      const key = path.resolve(account.automationUserDataDir);
      const context = sharedContexts.get(key);
      if (context) { sharedContexts.delete(key); activeContexts.delete(context); await context.close(); }
    };
      ...
      const result = await runWithRecentNotAvailableCache(target, siteState, config, async () => {
        ...
        if (nativeWafOrigins.has(target.origin)) {
          return withRetrySchedule({
            status: "deferred",
            ...nativePreflightFailure(preflight),
            url: preflight?.url || target.candidates?.[0] || target.origin,
            attempt: 1,
            nativePreflight: true,
          }, config);
    ...
    const runIsolatedPrimaryTarget = async (target) => {
      ...
      const started = Date.now();
      await releaseBoundAccountContext(account);
    ...
    for (let round = 0; round < recoveryRounds; round += 1) {
      ...
      for (const resultIndex of recoveryIndexes) {
        ...
        const identityBoundMethods = config.oauthExecutionAccountBindings?.[current.origin]
          ? methods.filter(method => ["oauth", "native_oauth"].includes(method.method))
          : methods;
        for (const method of identityBoundMethods) {
          try {
            ...
            if (["oauth_rate_limited", "oauth_upstream_unavailable", "oauth_callback_rejected"].includes(failedOutcome.failureCode)) {
      ...
      for (let recoveryIndex = 0; recoveryIndex < recoveryIndexes.length; recoveryIndex += 1) {
          ...
          } else if (loginRecovery?.terminalLoginFailure === "oauth_callback_rejected") {
            recoveredResult = {
              status: "needs_attention", reason: "站点拒绝 OAuth 回调，需修复认证流程；未重复提交凭据",
              failureCode: "oauth_callback_rejected", retryable: false, url: initialResult.url,
            };
    ...
    for (let accountIndex = 0; accountIndex < selectedSupplementalAccounts.length; accountIndex += 1) {
      ...
      const shouldReuse = prior && !(config.executionEngine === 'v1' && prior.availabilityKind === 'task_disabled') && (isTerminalResult(prior)
        || (!explicitSelection && !isRetryEligible(prior)));
      // Supplemental accounts bypass runOneTarget; apply the same explicit
      // account disable policy before opening their browser profile.
      const configuredSkip = configuredTargetSkip(account, config);
      if (configuredSkip) {
        results.push({
          ...account,
          ...configuredSkip,
