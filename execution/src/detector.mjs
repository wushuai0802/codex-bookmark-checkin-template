...
export function classifyPageText({ url = "", title = "", bodyText = "", hasPassword = false, challengeSelectors = false }) {
  ...
  if (/(今日已签到|今天已签到|今天已经签到过|已经签到|已完成签到|已签到|已簽到|查看(?:签到|簽到)(?:记录|記錄).{0,20}\d{1,2}(?:点|點)|無需重複簽到|无需重复签到|codex\s*(?:权益|權益)\s*已(?:领取|領取)|already checked[ -]?in|checked in today)/i.test(text)) {
...
export function ptPageEvidence({origin,url,bodyText,status,now=new Date(),allowUndatedActionText=true}={}) {
  ...
  const event=/(?:已签到|已簽到|已经签到|已經簽到|签到成功|簽到成功|successfully checked[ -]?in)/gi;
  const signals=[...text.matchAll(positive),...text.matchAll(event)].some(match=>{
    ...
    return /(?:今日|今天|当日).{0,18}(?:已签到|已簽到|已经签到|已經簽到|签到成功|簽到成功)|(?:已签到|已簽到|已经签到|已經簽到|签到成功|簽到成功).{0,18}(?:今日|今天|当日)|already checked[ -]?in today|checked in today/i.test(line)||dated.test(line)||
      (allowUndatedActionText&&/本次(?:签到|簽到).{0,18}(?:获得|獲得)/i.test(line));
  });
