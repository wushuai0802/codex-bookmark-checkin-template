...
test("私有运行版同步默认只报告，Apply 必须逐文件选择并备份原子替换", async () => {
  ...
  try {
    ...
    ], { encoding: "utf8" }), /requires explicit -IncludePaths/);
    assert.match(await fs.readFile(path.join(fixture.privateRoot, "src", "sample.mjs"), "utf8"), /value = 1/);

    const { stdout } = await execFileAsync(powershell, [
      "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", fixture.script,
      "-PrivateRoot", fixture.privateRoot, "-Apply", "-IncludePaths", "src\\sample.mjs",
    ], { encoding: "utf8" });
...
test("私有运行版同步通用默认项但保留私有配置", async () => {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(powershell, [
      "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", fixture.script,
      "-PrivateRoot", fixture.privateRoot, "-Apply", "-IncludePaths", "config\\defaults.json,requirements-ocr.txt,src\\sample.mjs",
    ], { encoding: "utf8" });
    const report = JSON.parse(stdout.trim());
...
test("私有运行版同步拒绝未登记文件和运行中的目标", async () => {
  const fixture = await makeFixture();
  try {
    await assert.rejects(execFileAsync(powershell, [
      "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", fixture.script,
      "-PrivateRoot", fixture.privateRoot, "-Apply", "-IncludePaths", "..\\outside.mjs",
    ], { encoding: "utf8" }), /pending managed files exactly/);
    await fs.mkdir(path.join(fixture.privateRoot, "tmp"));
    await fs.writeFile(path.join(fixture.privateRoot, "tmp", "run.lock"), "busy");
    await assert.rejects(execFileAsync(powershell, [
      "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", fixture.script,
      "-PrivateRoot", fixture.privateRoot, "-Apply", "-IncludePaths", "src\\sample.mjs",
    ], { encoding: "utf8" }), /run lock/);
    assert.match(await fs.readFile(path.join(fixture.privateRoot, "src", "sample.mjs"), "utf8"), /value = 1/);
  } finally {
    await fs.rm(fixture.root, { recursive: true, force: true });
  }
