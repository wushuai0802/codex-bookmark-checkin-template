...
test("New API accounts preserve their explicit mode and nonretryable result", () => {
  const [account] = configuredSupplementalOAuthAccounts(configWith([supplemental({ checkinMode: "new_api" })]), root);
  assert.equal(account.checkinMode, "new_api");
  assert.throws(() => configuredSupplementalOAuthAccounts(configWith([supplemental({ checkinMode: "typo" })]), root), /checkinMode/);
  const result = oauthHelperResultToCheckin(account, {
    dailyCheckin: { status: "needs_attention", failureCode: "submission_outcome_unknown", retryable: false },
  });
  assert.equal(result.failureCode, "submission_outcome_unknown");
  assert.equal(result.retryable, false);
  assert.equal(result.accountId, "200");
});
...
test("补充账号也经过显式账号停用保护", async () => {
  const source = await fs.readFile(path.join(root, "src", "index.mjs"), "utf8");
  assert.match(source, /const configuredSkip = configuredTargetSkip\(account, config\)/);
  assert.match(source, /if \(configuredSkip\) \{[\s\S]{0,500}candidateHistory: \[\]/);
});
