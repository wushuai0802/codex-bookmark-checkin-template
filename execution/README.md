...
这是统一签到仓库中的 `execution/` 包。以下命令默认在该目录运行；
仓库根目录的 `README.md` 说明与观测层、Harvest 和面板的协作方式。
...
2. 克隆统一仓库，并在 `execution/` 中完成执行层配置。
...
如果预检发现多个同名上级文件夹，应同时把预检返回的完整路径和 GUID 写入本机
`config/config.local.json` 的 `mobileFolderPaths`、`mobileFolderIds` 或
`mobileFolderGuids`。配置了任一精确选择器后，运行器只读取匹配的目录；路径或
GUID 发生变化会停止并报警，不会合并同名目录，也不会使用旧备份“补回”已删除的站点。
...
- 若单站登录需要现有本机代理的 SOCKS4 本地解析，可在私有配置中用 `nativeLoginProxyByOrigin` 绑定 HTTPS 来源与 `socks4://127.0.0.1:端口`。仅在单一来源、已绑定账号的专用 Profile 窗口使用；代理作用于该窗口进程（包括它打开的上游 OAuth 页面），不修改全局代理，也不关闭 TLS 校验。
...
- Windows 计划任务从签到时间起按半小时做无副作用探测，用户级调度器按分钟探测；用户级模式由注册表与用户“启动”文件夹双入口、独立守护、PowerShell 看门狗和调度器恢复，两种模式都受每日次数上限和运行锁保护。
- 计划任务和看门狗通过 `wscript.exe` GUI 启动器承接 PowerShell，避免交互桌面上出现瞬时控制台窗口；启动器只负责隐藏转发，不改变签到进程锁和退出码。
