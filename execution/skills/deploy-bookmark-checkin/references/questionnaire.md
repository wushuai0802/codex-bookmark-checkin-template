...
2. Bookmark scope, before every other preference: Chrome profile name or Auto; optional Edge profile name, Auto, or None; Chrome parent/target folder names; and, when different, Edge parent/target folder names. Offer candidates found by preflight, but do not preselect or infer names. When a parent name appears more than once, require the selected candidate's complete path and stable GUID/ID; configure exact selectors so the sources are not merged implicitly.
