...
if (-not (Test-Path -LiteralPath $hiddenLauncher -PathType Leaf)) { throw "缺少无控制台启动器：$hiddenLauncher" }
$wscript = (Get-Command wscript.exe -ErrorAction Stop).Source
$arguments = "//B //NoLogo `"$hiddenLauncher`" `"$shell`" `"$schedulerScript`" `"$root`" -Once"
