...
$patterns = [ordered]@{
    ...
    'Email address' = '(?i)\b[A-Z0-9._%+-]+@(?!(?:example\.com|[A-Z0-9.-]+\.example)\b)[A-Z0-9.-]+\.[A-Z]{2,}\b'
...
foreach ($relative in $relativeFiles) {
    ...
    foreach ($line in Get-Content -LiteralPath $fullPath -ErrorAction SilentlyContinue) {
        ...
        foreach ($entry in $patterns.GetEnumerator()) {
            if ($relative -match '(^|/)scripts/Scan-PublicSafety\.ps1$' -and $entry.Key -eq 'Assigned secret') { continue }
