...
$hiddenLauncher = Join-Path $PSScriptRoot 'Run-HiddenPowerShell.vbs'
...
$latestStoredResultComplete = $null -ne $latest `
...
$latestResultValid = $latestRunToday -and $latestStoredResultComplete
...
$latestStoredExecutionComplete = $latestStoredResultComplete -and ($null -eq $latest.executionComplete -or $latest.executionComplete -eq $true)
$latestStoredBusinessComplete = Get-CheckinBusinessComplete -LatestExecutionComplete $latestStoredExecutionComplete -ProblemCount ([int]$problemCount)
$serializedBusinessCompleteMatches = $null -eq $latest -or (Test-SerializedCheckinBusinessComplete `
    ...
    -ComputedBusinessComplete $latestStoredBusinessComplete)
...
$oauthExecutionBindingsReady = @(
    if ($config.oauthExecutionAccountBindings) {
        $config.oauthExecutionAccountBindings.PSObject.Properties | Where-Object {
            $origin = [string]$_.Name
            $key = [string]$_.Value
            $matched = @($oauthRecoveryAccountBindings | Where-Object { $_.origin -eq $origin -and $_.accountKey -eq $key -and $_.ready })
            $shared = Get-OAuthHealthMapValue $config.oauthSiteSessionBindings $origin
            $isolated = Get-OAuthHealthMapValue $config.isolatedOAuthSiteProfiles $origin
            $matched.Count -ne 1 -or [bool]$shared -or [bool]$isolated
        }
    }
).Count -eq 0
...
# `ReadAndExecute,Synchronize` is a normal read-only rule.  Do not use the
# aggregate `FullControl` enum as a bit mask here because it includes the
# `Synchronize` bit and falsely classifies read-only principals as writers.
$writeCapabilityMask = [System.Security.AccessControl.FileSystemRights]::WriteData -bor `
...
$wideWriteRules = @($rootAcl.Access | Where-Object {
    ...
    $isFullControl = ($rights -band [System.Security.AccessControl.FileSystemRights]::FullControl) -eq [System.Security.AccessControl.FileSystemRights]::FullControl
    return $sid -notin $allowedSids -and ($isFullControl -or (($rights -band $writeCapabilityMask) -ne 0))
})
...
$scheduledTaskActionValid = if ($scheduledTask) {
    @($scheduledTask.Actions | Where-Object {
        ...
        $execute -match '(?i)wscript(?:\.exe)?$' -and
            $arguments -like "*$hiddenLauncher*" -and
            $arguments -like "*$schedulerScript*"
    }).Count -eq 1
} else { $false }
...
$hiddenLauncherPresent = Test-Path -LiteralPath $hiddenLauncher -PathType Leaf
$scheduledTaskReady = $scheduledTask -and [string]$scheduledTask.State -ne 'Disabled' -and $scheduledTaskActionValid -and $hiddenLauncherPresent -and $scheduledTaskTriggerFrequencyValid
$userSchedulerConfigured = $startupEntryPresent
# A trigger-frequency defect does not change which scheduler owns execution.
# Report that defect on the Windows task instead of diagnosing dormant legacy
# watchdogs as the active scheduler.
$windowsTaskOwnsExecution = $scheduledTask -and [string]$scheduledTask.State -ne 'Disabled' -and $scheduledTaskActionValid
$useUserScheduler = $userSchedulerConfigured -and -not $windowsTaskOwnsExecution
...
$checks = [ordered]@{
    ...
    oauthExecutionBindingsReady = [bool]$oauthExecutionBindingsReady
    ...
    schedulerHiddenLauncherPresent = [bool]$hiddenLauncherPresent
