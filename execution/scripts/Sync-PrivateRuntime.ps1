...
$requestedPaths = @($IncludePaths | ForEach-Object { [string]$_ -split ',' } | ForEach-Object { $_.Trim() } | Where-Object { $_ })
$selected = @()
$backupRoot = $null
if ($Apply -and $pending.Count -gt 0) {
    if ($requestedPaths.Count -eq 0) { throw 'Apply requires explicit -IncludePaths; bulk replacement is disabled' }
    if (@($requestedPaths | Sort-Object -Unique).Count -ne $requestedPaths.Count) { throw 'IncludePaths contains duplicates' }
    $selected = @($pending | Where-Object { $requestedPaths -contains $_.relativePath })
    if ($selected.Count -ne $requestedPaths.Count) { throw 'IncludePaths must name pending managed files exactly' }
    if (Test-Path -LiteralPath (Join-Path $resolvedPrivateRoot 'tmp\run.lock')) {
        throw 'Private runtime has an active or unrecovered run lock'
    }
...
$result = [ordered]@{
    ...
    remainingCount = $pending.Count - $selected.Count
    ...
    appliedPaths = @($selected | ForEach-Object { $_.relativePath })
