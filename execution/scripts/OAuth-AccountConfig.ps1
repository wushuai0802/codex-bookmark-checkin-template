...
function Resolve-OAuthAccountConfiguration([object]$Config, [string]$Root, [string]$AccountKey) {
    ...
    foreach ($property in @($Config.oauthAccountIdentities.PSObject.Properties)) {
        ...
        $matches += [pscustomobject]@{
            ...
            CheckinMode = if ($identity.checkinMode) { [string]$identity.checkinMode } else { 'oauth_relogin' }
    ...
    foreach ($account in @($Config.supplementalOAuthAccounts)) {
        ...
        $matches += [pscustomobject]@{
            ...
            CheckinMode = if ($account.checkinMode) { [string]$account.checkinMode } else { 'oauth_relogin' }
    ...
    $binding = $matches[0]
    if ($binding.CheckinMode -notin @('oauth_relogin', 'new_api')) { throw 'OAuth checkinMode 无效。' }
