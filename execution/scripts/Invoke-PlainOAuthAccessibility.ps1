...
public static class OAuthProfileWindows {
...
function Get-ChromeAutomationRoots {
    ...
    @(
        [OAuthProfileWindows]::Find([int[]]$ids) | ForEach-Object {
            try {
                $handle = [IntPtr]$_
                if (-not $preparedHandles.ContainsKey($handle)) {
                    # An offscreen Chrome window may have no MainWindowHandle.
                    # Render only this isolated profile before reading UIA.
                    if (-not [OAuthProfileWindows]::RenderOffscreen($handle)) { return }
