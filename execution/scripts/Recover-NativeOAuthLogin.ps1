...
$checkinMode = $binding.CheckinMode
...
if ($checkinMode -ne 'new_api' -and ($null -eq $rule -or $rule.nativeBrowser -ne $true)) {
...
function Invoke-NativeOAuthRound {
    ...
    try {
        ...
        $roundOutput = @(& $node (Join-Path $root 'src\native-oauth-login.mjs') $debugPort $originValue $Provider $ExpectedAccountId $AccountKey $AccountLabel $UpstreamProvider $targetUrl.AbsoluteUri $checkinMode)
...
$firstResult = Get-LastOAuthJsonResult $firstRound.Output
if ($checkinMode -eq 'new_api' -and ($firstResult.dailyCheckin -or $firstResult.failureCode -eq 'account_mismatch')) {
