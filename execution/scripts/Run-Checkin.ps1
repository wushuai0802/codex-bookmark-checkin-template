...
. (Join-Path $PSScriptRoot 'V2-EngineGateway.ps1')
if (Invoke-V2EngineGateway -Mode 'execute' -Parameters $PSBoundParameters) { exit $script:V2EngineExitCode }
$AccountKeys = @($AccountKeys | ForEach-Object { [string]$_ -split ',' } | Where-Object { $_ })
$Origins = @($Origins | ForEach-Object { [string]$_ -split ',' } | Where-Object { $_ })
